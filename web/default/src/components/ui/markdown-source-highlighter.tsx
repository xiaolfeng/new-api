...
function highlightLine(line: string, lineIndex: number): React.ReactNode[] {
  ...
  if (headingMatch) {
    ...
    elements.push(
      <strong
        key={`heading-text-${lineIndex}`}
        className='text-foreground font-bold'
      >
        {highlightSegment(
          line.slice(headingMatch[0].length),
          `h${headingMatch[1].length}-${lineIndex}`
        )}
      </strong>
    )
    return elements
  ...
  if (ulMatch) {
    ...
    elements.push(
      ...highlightSegment(line.slice(ulMatch[0].length), `ul-${lineIndex}`)
    )
    return elements
  }
  ...
  if (olMatch) {
    ...
    elements.push(
      ...highlightSegment(line.slice(olMatch[0].length), `ol-${lineIndex}`)
    )
    return elements
  }
  ...
  if (quoteMatch) {
    elements.push(
      <span key={`quote-indent-${lineIndex}`}>{quoteMatch[1]}</span>
    )
    elements.push(
    ...
    elements.push(
      <span key={`quote-text-${lineIndex}`} className='text-gray-500 italic'>
        {highlightSegment(
          line.slice(quoteMatch[0].length),
          `quote-${lineIndex}`
        )}
      </span>
    )
...
export function MarkdownSourceHighlighter({
  ...
  const highlighted = useMemo(() => {
    ...
    for (let lineNum = 0; lineNum < lines.length; lineNum++) {
      ...
      if (codeBlockMatch) {
        if (!inCodeBlock) {
          ...
          for (let ci = 0; ci < codeBlockLines.length; ci++) {
            result.push(
              <div
                key={`cb${codeBlockStartLine}-cl${ci}`}
                className='font-mono'
              >
                {codeBlockLines[ci] || '\u00A0'}
              </div>
      ...
      result.push(
        <div key={`mdl-${lineNum}`}>{highlightLine(line, lineNum)}</div>
      )
    }
