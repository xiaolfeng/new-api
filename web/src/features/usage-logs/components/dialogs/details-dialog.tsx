...
function StructuredLogContent(props: { sections: ParsedSections }) {
  const { t } = useTranslation();
        ...
        >
          <div className="relative space-y-2">
            ...
            {sections.requestBlocks.map((block, index) => (
              <div
                key={`${block.type}-${block.role}-${block.text}`}
        ...
        <ScrollArea className="min-h-0 flex-1 overflow-hidden">
          <div className="space-y-3 p-4">
            {isStructured && sections ? (
              <StructuredLogContent sections={sections} />
            ) : (
            ...
            {blocks.map((block) => (
              ...
              >
                <div className="relative">
                  ...
                  <pre className="bg-background/60 max-h-64 overflow-auto rounded-md border p-2 font-mono text-xs break-all whitespace-pre-wrap">
                    {block.content}
                  </pre>
                </div>
              </DetailSection>
...
export function DetailsDialog(props: DetailsDialogProps) {
  ...
  const parsedDetailRecord = useMemo(() => {
    if (!detailRecord) return null;
    try {
      const parsed = JSON.parse(detailRecord) as LogDetailRecord;
      if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
        return null;
      }
      return parseLogDetailRecord(parsed);
    } catch {
      return null;
    }
  }, [detailRecord]);
  const requestHeaders = parsedDetailRecord?.headers ?? {};
  ...
  let activeSheetContent = "";
  if (detailSheet === "record") {
    activeSheetContent = detailRecord;
  } else if (detailSheet === "full_log") {
    activeSheetContent = fullLog;
  }

  let activeSheetTitle = "";
  if (detailSheet === "record") {
    activeSheetTitle = t("Detailed Record");
  } else if (detailSheet === "full_log") {
    activeSheetTitle = t("Full Log");
  } else if (detailSheet === "bamboo_debug") {
    activeSheetTitle = t("Bamboo Debug");
  }

  let activeSheetDescription = "";
  if (detailSheet === "record") {
    activeSheetDescription = t("Structured request, tool and response details");
  } else if (detailSheet === "full_log") {
    activeSheetDescription = t("Complete raw request and response payload");
  } else if (detailSheet === "bamboo_debug") {
    activeSheetDescription = t("Bamboo relay debug information");
  }
                      ...
                      <div className="mt-1.5 border-t pt-1.5">
                        <div className="flex items-center gap-2 text-xs">
                          ...
                          <span className="text-foreground ml-auto font-mono font-medium tabular-nums">
                            {(other.bamboo_timing.total_ms / 1000).toFixed(2)}s
                          </span>
                        </div>
                      </div>
                    )}
              ...
              <dl className="space-y-1">
                {Object.entries(requestHeaders).map(([key, value]) => (
                  ...
                  >
                    ...
                    <dd className="min-w-0 flex-1 font-mono text-[11px] break-all">
                      {value}
                    </dd>
                  </div>
                ))}
