...
func TestResponsesRequestToChatCompletionsRequestMapsDeveloperRole(t *testing.T) {
	got, err := ResponsesRequestToChatCompletionsRequest(&dto.OpenAIResponsesRequest{
		Model: "gpt-test",
		Input: mustRawMessage(t, []map[string]any{
...
func TestResponsesRequestToChatCompletionsRequestEnablesLogProbs(t *testing.T) {
	topLogProbs := 5
	got, err := ResponsesRequestToChatCompletionsRequest(&dto.OpenAIResponsesRequest{
		Model:       "gpt-test",
		Input:       mustRawMessage(t, []map[string]any{{"role": "user", "content": "hi"}}),
...
func TestResponsesRequestToChatCompletionsRequestExpandsNamespaceTools(t *testing.T) {
	got, err := ResponsesRequestToChatCompletionsRequest(&dto.OpenAIResponsesRequest{
		Model: "gpt-test",
		Input: mustRawMessage(t, []map[string]any{{"role": "user", "content": "test"}}),
		Tools: mustRawMessage(t, []map[string]any{
			{
				"type":        "function",
				"name":        "get_weather",
			...
			},
			{
				...
				"tools": []map[string]any{
					{"type": "function", "name": "doc_list", "description": "List documents"},
					{"type": "function", "name": "doc_detail", "description": "Get document"},
					{"type": "tool_search"},
				},
			},
			{"type": "web_search"},
	...
	assert.Equal(t, "doc_list", got.Tools[1].Function.Name)
	assert.Equal(t, "doc_detail", got.Tools[2].Function.Name)
...
func TestResponsesRequestToChatCompletionsRequestStripsOptionalUnsupportedFields(t *testing.T) {
	got, err := ResponsesRequestToChatCompletionsRequest(&dto.OpenAIResponsesRequest{
		Model:        "gpt-test",
		Input:        mustRawMessage(t, []map[string]any{{"role": "user", "content": "hi"}}),
		Include:      mustRawMessage(t, []string{"message"}),
		Truncation:   mustRawMessage(t, "auto"),
		MaxToolCalls: lo.ToPtr(uint(5)),
		Preset:       mustRawMessage(t, "default"),
	})
	require.NoError(t, err)
...
func TestResponsesRequestToChatCompletionsRequestMergesEmptyAssistantWithFunctionCall(t *testing.T) {
	got, err := ResponsesRequestToChatCompletionsRequest(&dto.OpenAIResponsesRequest{
		Model: "gpt-test",
		Input: mustRawMessage(t, []map[string]any{
