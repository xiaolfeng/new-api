...
func ResponsesRequestToChatCompletionsRequest(req *dto.OpenAIResponsesRequest) (*dto.GeneralOpenAIRequest, error) {
	...
	if req.TopLogProbs != nil {
		// Chat Completions 要求 top_logprobs 与 logprobs 联动，否则上游会拒绝。
		logProbs := true
...
func responsesInputItemToChatMessages(item map[string]any, messages []dto.Message) ([]dto.Message, error) {
	...
	case "":
		role = "user"
	case "developer":
		// Responses developer 在 Chat Completions 中无对应角色，语义等价 system。
		role = "system"
	}
...
func responsesRequestToolsToChat(raw json.RawMessage) ([]dto.ToolCallRequest, error) {
	...
	for _, tool := range tools {
		...
		case "namespace":
			// MCP namespace：展开内部 function 工具；tool_search 等子项与空 namespace 忽略。
			...
			for _, rawSub := range subTools {
				...
				if !ok || strings.TrimSpace(kitutil.Interface2String(sub["type"])) != "function" {
					continue
				}
				out = append(out, dto.ToolCallRequest{
					Type: "function",
					Function: dto.FunctionRequest{
						Name:        strings.TrimSpace(kitutil.Interface2String(sub["name"])),
						Description: kitutil.Interface2String(sub["description"]),
						Parameters:  sub["parameters"],
					},
				})
			}
		default:
			// web_search 等 Responses 内置工具在 Chat Completions 中无对应类型，忽略以免被拒。
		}
	}
	return out, nil
}
...
func responsesImagePartToChatImageURL(part map[string]any) any {
	if imageURL, ok := part["image_url"]; ok {
		...
		case string:
			// Chat Completions 要求 image_url 为对象；Responses 允许字符串。
			wrapped := map[string]any{"url": v}
