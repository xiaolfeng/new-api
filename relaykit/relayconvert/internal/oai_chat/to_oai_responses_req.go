...
func convertLegacyChatFunctionsToResponsesTools(functions json.RawMessage) json.RawMessage {
	...
	if err := kitutil.Unmarshal(functions, &legacyFunctions); err != nil || len(legacyFunctions) == 0 {
		return nil
	}
	...
	for _, fn := range legacyFunctions {
		...
		if description := strings.TrimSpace(kitutil.Interface2String(fn["description"])); description != "" {
...
func convertLegacyFunctionCallToResponsesToolChoice(functionCall json.RawMessage) json.RawMessage {
	...
	if err := kitutil.Unmarshal(functionCall, &choiceString); err == nil {
		choiceString = strings.TrimSpace(choiceString)
		if choiceString == "" {
			return nil
		}
		toolChoiceRaw, _ := kitutil.Marshal(choiceString)
		return toolChoiceRaw
	}

	var choice map[string]any
	if err := kitutil.Unmarshal(functionCall, &choice); err != nil || len(choice) == 0 {
		return nil
	}
