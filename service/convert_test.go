...
func TestResponseConverterFacades(t *testing.T) {
	cache5m, cache1h := relayconvert.NormalizeCacheCreationSplit(10, 3, 2)
	...
	claudeResp := relayconvert.ResponseOpenAI2Claude(chatResp, &relaycommon.RelayInfo{})
	require.NotNil(t, claudeResp)
	assert.Equal(t, "message", claudeResp.Type)

	geminiResp := relayconvert.ResponseOpenAI2Gemini(chatResp, &relaycommon.RelayInfo{})
...
func TestStreamResponseConverterFacades(t *testing.T) {
	...
	claudeResponses := relayconvert.StreamResponseOpenAI2Claude(streamResp, info)
	require.NotEmpty(t, claudeResponses)

	geminiResp := relayconvert.StreamResponseOpenAI2Gemini(streamResp, &relaycommon.RelayInfo{})
	require.NotNil(t, geminiResp)
	require.Len(t, geminiResp.Candidates, 1)
}
