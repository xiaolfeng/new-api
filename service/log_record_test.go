...
func TestBuildLogRecordOpenAIStreamStructuredBlocks(t *testing.T) {
	...
	require.Equal(t, "用户想", record.OpenAIResponseBlocks[0].Content)
	require.Equal(t, "tool_call", record.OpenAIResponseBlocks[1].Type)
	require.Equal(t, "call_1", record.OpenAIResponseBlocks[1].ID)
	require.Equal(t, "fetch_fetch", record.OpenAIResponseBlocks[1].Name)
	require.Equal(t, map[string]any{"url": "https://httpbin.org/get"}, record.OpenAIResponseBlocks[1].Arguments)
	require.Equal(t, "content", record.OpenAIResponseBlocks[2].Type)
	require.Equal(t, "嘿呀~", record.OpenAIResponseBlocks[2].Content)
