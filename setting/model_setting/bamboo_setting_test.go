...
func TestBambooClientProfileFlagDefaultsMissingKeys(t *testing.T) {
	...
	assert.True(t, st.ClientStrictEgressEnabled())
...
func TestBambooClientProfileFlagJSONOverrides(t *testing.T) {
	st := cloneBambooBools(defaultBambooSettings)
	require.NoError(t, common.Unmarshal([]byte(`{"enable_client_strict_egress":false}`), &st))
	assert.False(t, st.ClientStrictEgressEnabled())
	assert.False(t, st.ClientReturnProfilesEnabled())
...
func TestBambooClientProfileFlagPointerIsolation(t *testing.T) {
	...
	require.NotNil(t, live.EnableClientStrictEgress)
	require.NotSame(t, defaultBambooSettings.EnableClientStrictEgress, live.EnableClientStrictEgress)
	require.NotSame(t, defaultBambooSettings.EnableClientReturnProfiles, live.EnableClientReturnProfiles)

	*live.EnableClientStrictEgress = false
	cloned := cloneBambooBools(defaultBambooSettings)
	assert.True(t, cloned.ClientStrictEgressEnabled())
	assert.True(t, defaultBambooSettings.ClientStrictEgressEnabled())
}
...
func TestBambooClientProfileFlagNilHelpers(t *testing.T) {
	...
	assert.True(t, empty.ClientStrictEgressEnabled())
