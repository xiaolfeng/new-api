...
func responsesViaChatCompletions(c *gin.Context, info *relaycommon.RelayInfo, adaptor channel.Adaptor, responsesReq *dto.OpenAIResponsesRequest) (*dto.Usage, *types.NewAPIError) {
	chatReq, err := relayconvert.ResponsesRequestToChatCompletionsRequest(responsesReq)
	if err != nil {
	...
	if info.IsStream {
		usage, newApiErr := openaichannel.ChatCompletionsStreamToResponsesHandler(c, info, httpResp, responsesReq)
		if newApiErr != nil {
			service.ResetStatusCode(newApiErr, statusCodeMappingStr)
			return nil, newApiErr
		}
	...
	usage, newApiErr := openaichannel.ChatCompletionsToResponsesHandler(c, info, httpResp, responsesReq)
	if newApiErr != nil {
		service.ResetStatusCode(newApiErr, statusCodeMappingStr)
		return nil, newApiErr
	}
